...
import { browser__build, type relysjs__server__build_config_T, server__build } from 'relysjs'
import { config__init } from './app/index.js'
export async function build(config?:relysjs__server__build_config_T) {
	config__init()
...
if (is_entry_file_(import.meta.url, process.argv[1])) {
	await build({ relysjs: { app__start: false }})
}
